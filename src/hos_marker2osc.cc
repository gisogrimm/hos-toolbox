...
namespace HoS {
  ...
  */
  ...
  public:
    oscam_t(const std::string& oscad, const std::string& op,
            const std::string& fname, const std::string& jackname,
            bool markerpath);
    void run();
...
*/
oscam_t::oscam_t(const std::string& oscad, const std::string& op,
                 const std::string& fname, const std::string& jackname,
                 bool markerpath)
    : jackc_transport_t(jackname),
      // b_quit(false),
      b_markerpath(markerpath), lo_addr(lo_address_new_from_url(oscad.c_str())),
      path(op)
{
  // DEBUG(fname);
  ...
  if(root) {
    ...
    for(xmlpp::NodeSet::iterator nit = nodesPiece.begin();
        nit != nodesPiece.end(); ++nit) {
      xmlpp::Element* loc = dynamic_cast<xmlpp::Element*>(*nit);
      if(loc) {
        marker_t m(atol(loc->get_attribute_value("start").c_str()),
                   loc->get_attribute_value("name"));
        if(b_markerpath)
          m.name = path + m.name;
        db.push_back(m);
      }
...
int oscam_t::process(jack_nframes_t nframes,
...
int main(int argc, char** argv)
{
  ...
  struct option long_options[] = {
      {"dest", 1, 0, 'd'},     {"pathprefix", 1, 0, 'p'},
      {"jackname", 1, 0, 'n'}, {"markerpath", 0, 0, 'm'},
      {"help", 0, 0, 'h'},     {0, 0, 0, 0}};
  int opt(0);
  int option_index(0);
