...
mm_hdsp_t::mm_hdsp_t() : mm(NULL), modified(true), b_run_service(false)
{
...
mm_midicc_t::mm_midicc_t()
    : midi_ctl_t("mm_mididcc"), mm(NULL), modified(true), b_shift1(false),
      b_shift2(false), selected_out(0), b_run_service(false)
{
  pthread_mutex_init(&mutex, NULL);
  start_service();
  start_updt_service();
...
mm_window_t::mm_window_t() : m_Box(Gtk::ORIENTATION_VERTICAL), mm(NULL)
{
  set_title("main menu example");
  set_default_size(200, 200);
  add(vbox);
  vbox.add(
      m_Box); // put a MenuBar at the top of the box and other stuff below it.
  vbox.add(mixer);
  ...
  m_refActionGroup->add(
      Gtk::Action::create("FileReload", Gtk::Stock::REVERT_TO_SAVED),
