...
tmcm_error::tmcm_error(uint8_t err_no) : errno_(err_no) {}

const char* tmcm_error::what() const throw()
{
  switch(errno_) {
  case 101:
    return "Command loaded into TMCL program error";
  case 1:
...
tmcm6110_t::tmcm6110_t() : device(-1), module_addr(1)
{
...
uint32_t tmcm6110_t::write_cmd(uint8_t command_no, uint8_t type_no,
                               uint8_t motor_no, uint32_t value)
{
  uint32_t retv_value(0);
  pthread_mutex_lock(&mtx);
  try {
    // printf("%d: type=%d motor=%d
    ...
    if(n > 0) {
      ...
      if(rlen == 9) {
        retv = data[2];
        uint32_t rv(0x1000000 * data[4] + 0x10000 * data[5] + 0x100 * data[6] +
                    data[7]);
        retv_value = rv;
        if(retv != 100) {
