...
namespace HoS {
  ...
  */
  class pos_interp_t {
  public:
    pos_interp_t(unsigned int dt):dt1(1.0f/(float)dt),x(0.0f),y(0.0f),z(0.0f){set(0.0f,0.0f,0.0f);};
...
int sphere_xyz_t::process(jack_nframes_t nframes,const std::vector<float*>& inBuffer,const std::vector<float*>& outBuffer)
{
