...
{
  HoS::wave_t zero1(zpad1,long_windowed_in.b);
  HoS::wave_t zero2(zpad2,&(long_windowed_in.b[zpad1+wndlen_]));
  HoS::wave_t newchunk(wndlen_,&(long_windowed_in.b[zpad1]));
  ...
  for(unsigned int k=0;k<wndlen_;k++)
    newchunk[k] = window.b[k]*long_in.b[k];
