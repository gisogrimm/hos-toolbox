...
int _marker(const char* path, const char* types, lo_arg** argv, int argc,
            lo_message msg, void* user_data)
{
  char cmd[1024];
  char* m;
  if((argc == 1) && (types[0] == 's')) {
    ...
    if(strncmp(m, "preset:", 7) == 0) {
      sprintf(cmd,
              "bash -c \"cat presets/%s.tcp | hos_sendosc "
              "osc.tcp://localhost:9877/\"",
              m + 7);
      int err = system(cmd);
      if(err != 0) {
        DEBUG(err);
      ...
      sprintf(cmd,
              "bash -c \"cat presets/%s.udp | hos_sendosc "
              "osc.udp://localhost:9877/\"",
              m + 7);
      err = system(cmd);
      if(err != 0) {
        DEBUG(err);
