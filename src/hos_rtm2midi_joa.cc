...
midi_note_t::midi_note_t()
    : velocity(0), pitch(0), startcnt(0), lengthcnt(0), voice(0), process(false)
{
}
...
int midi_t::add_note(const char* path, const char* types, lo_arg** argv,
                     int argc, lo_message msg, void* user_data)
{
  if(user_data && (argc == 4))
    ((midi_t*)user_data)
...
int midi_t::set_time(const char* path, const char* types, lo_arg** argv,
                     int argc, lo_message msg, void* user_data)
{
...
void midi_t::add_note(unsigned int voice, int pitch, unsigned int length,
                      double time)
{
  note_t n;
  n.voice = voice;
...
void midi_t::add_note(const note_t& n)
{
  if(n.pitch == PITCH_REST)
    return;
  for(std::vector<midi_note_t>::iterator note = notes.begin();
      note != notes.end(); ++note)
    if(!(note->process)) {
      ...
      if(note->process) {
        if(note->startcnt) {
          ...
          if(!(note->startcnt)) {
            ...
            buffer[1] = note->pitch;
            // std::cout << n << std::endl;
            buffer[0] = 0x90 + note->voice; /* note on */
          }
        } else {
          if(note->lengthcnt) {
            note->lengthcnt--;
            if(!(note->lengthcnt)) {
              buffer = jack_midi_event_reserve(port_buf, t, 3);
              buffer[2] = note->velocity;     /* velocity */
              buffer[1] = note->pitch;        /* note off */
              buffer[0] = 0x80 + note->voice; /* note on */
            }
          } else {
            ;
            note->process = false;
...
midi_t::midi_t(const std::string& name)
    : TASCAR::osc_server_t("239.255.1.7", "9877"), time(0),
      samples_per_brevis(48000), timecnt(0)
{
  notes.resize(512);
