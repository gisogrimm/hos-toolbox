...
int meter_t::setlevel(const char* path, const char* types, lo_arg** argv,
                      int argc, lo_message msg, void* user_data)
{
  if((argc == 3) && (types[0] == 'f') && (types[1] == 'f') &&
     (types[2] == 'f')) {
...
int meter_t::settarget(const char* path, const char* types, lo_arg** argv,
                       int argc, lo_message msg, void* user_data)
{
  if((argc == 1) && (types[0] == 'f')) {
...
meter_t::meter_t(const std::string& paddr, lo_server_thread& l, double lmin_,
                 double lmax_)
    : peak(0), rms_fast(0), rms_slow(0), lmin(lmin_), lmax(lmax_), target(80),
      lost(l)
{
  std::string baseaddr("/rmsmeter");
  std::string tmp;
  tmp = baseaddr + paddr;
  lo_server_thread_add_method(lost, tmp.c_str(), "fff", meter_t::setlevel,
                              this);
  tmp = baseaddr + "/target" + paddr;
  lo_server_thread_add_method(lost, tmp.c_str(), "f", meter_t::settarget, this);
  Glib::signal_timeout().connect(sigc::mem_fun(*this, &meter_t::on_timeout),
                                 50);
#ifndef GLIBMM_DEFAULT_SIGNAL_HANDLERS_ENABLED
  // Connect the signal handler if it isn't already a virtual method override:
  // signal_expose_event().connect(sigc::mem_fun(*this,
  // &meter_t::on_expose_event), false);
...
meter_scale_t::meter_scale_t(double lmin_, double lmax_)
    : lmin(lmin_), lmax(lmax_)
{
#ifndef GLIBMM_DEFAULT_SIGNAL_HANDLERS_ENABLED
  // Connect the signal handler if it isn't already a virtual method override:
  signal_expose_event().connect(sigc::mem_fun(*this, &meter_t::on_expose_event),
                                false);
#endif // GLIBMM_DEFAULT_SIGNAL_HANDLERS_ENABLED
}
...
bool meter_scale_t::on_expose_event(GdkEventExpose* event)
{
  ...
  if(window) {
    ...
    cr->set_source_rgb(0, 0, 0);
    char ctmp[128];
    for(double l = 10 * floor(0.1 * lmin); l <= 10 * floor(0.1 * lmax);
        l += 10) {
      cr->move_to(0, -l - 0.5);
      sprintf(ctmp, "%g", l);
      cr->show_text(ctmp);
