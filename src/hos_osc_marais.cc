...
public:
  osc_marais_t(const std::string& oscad,const std::string& multicast,const std::string& serverport,const std::string& jackname);
  ~osc_marais_t();
  void run();
  int process(jack_nframes_t nframes,const std::vector<float*>& inBuffer,const std::vector<float*>& outBuffer);
  static int osc_upload(const char *path, const char *types, lo_arg **argv, int argc, lo_message msg, void *user_data);
...
int osc_marais_t::osc_upload(const char *path, const char *types, lo_arg **argv, int argc, lo_message msg, void *user_data)
{
  if( user_data )
...
int osc_marais_t::process(jack_nframes_t nframes,const std::vector<float*>& inBuffer,const std::vector<float*>& outBuffer)
{
