...
namespace MM {
  ...
  */
  class mm_hdsp_t {
  public:
    mm_hdsp_t(const std::string& osc_server_addr,
...
mm_hdsp_t::mm_hdsp_t(const std::string& osc_server_addr,
                     const std::string& osc_server_port,
                     const std::string& osc_dest_addr)
    : mm(NULL), modified(true), b_exit(false)
{
  ...
  lo_server_thread_add_method(lost, "/hdspmm/backend_new", "ii",
                              mm_hdsp_t::hdspmm_new, this);
  lo_server_thread_add_method(lost, "/hdspmm/backend_quit", "",
                              mm_hdsp_t::hdspmm_destroy, this);
        ...
        for(unsigned int kinsub = 0; kinsub < inports.size(); kinsub++) {
          for(unsigned int koutsub = 0; koutsub < outports.size(); koutsub++) {
            ...
            snd_ctl_elem_value_set_integer(
                ctl, 2, (int)(32767 * std::min(1.0, std::max(0.0, val))));
...
int mm_hdsp_t::hdspmm_new(const char* path, const char* types, lo_arg** argv,
                          int argc, lo_message msg, void* user_data)
{
  // DEBUG(path);
...
int mm_hdsp_t::hdspmm_destroy(const char* path, const char* types,
                              lo_arg** argv, int argc, lo_message msg,
                              void* user_data)
{
  // DEBUG(path);
...
int main(int argc, char** argv)
{
  try {
    ...
    if((argc == 2) && (strcmp(argv[1], "-h") == 0)) {
      std::cerr << "Usage: mm_hdsp [ <osc_server_addr> <osc_server_port> "
                   "<osc_dest_url> ]\n";
      return 1;
    }
    // setup OSC server:
