...
int mainmix_t::process(jack_nframes_t nframes,
                       const std::vector<float*>& inBuffer,
                       const std::vector<float*>&)
{
  gain = inBuffer[0][0];
...
void mainmix_t::upload(float val)
{
  ...
  for(uint32_t k = 0; k < 26; k++) {
    ...
    snd_ctl_elem_value_set_integer(
        ctl, 2, (int)(32767 * std::min(1.0f, std::max(0.0f, val))));
    if((err = snd_ctl_elem_write(handle, ctl)) < 0) {
      fprintf(stderr, "Alsa error: %s\n", snd_strerror(err));
    }
  }
...
void usage(struct option* opt)
{
  std::cout << "Usage:\n\nhos_mainmix [options]\n\nOptions:\n\n";
  std::cout
      << "Read gain from jack port, create direct mapping in HDSP 9652 from\n"
...
int main(int argc, char** argv)
{
  ...
  struct option long_options[] = {
      {"help", 0, 0, 'h'}, {"jackname", 1, 0, 'j'}, {0, 0, 0, 0}};
  int opt(0);
  int option_index(0);
