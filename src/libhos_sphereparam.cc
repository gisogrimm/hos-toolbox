...
namespace OSC {
  ...
  // int handler_f(const char *path, const char *types, lo_arg **argv, int argc,
  ...
  int handler_angle_f(const char* path, const char* types, lo_arg** argv,
  ...
  int _feedbackaddr(const char* path, const char* types, lo_arg** argv,
                    int argc, lo_message msg, void* user_data)
  {
    if((argc == 1) && (types[0] == 's')) {
  ...
  int _setelev(const char* path, const char* types, lo_arg** argv, int argc,
               lo_message msg, void* user_data)
  {
    // DEBUG(1);
    if((argc == 1) && (types[0] == 'f')) {
...
par_t::par_t()
    : phi0(0), elev(0), rvbelev(0), random(0), f(0), r(1.0), width(18.0),
      rho_maxw(5.0), theta(0), e(0), f_epi(0), r_epi(0), phi0_epi(0), onset(0),
      randpos(0), g_in(1.0), g_rvb(1.0), map_octave(0), map_f(0)
{
}
...
parameter_t::parameter_t(const std::string& name)
    : TASCAR::osc_server_t(OSC_ADDR, OSC_PORT, "UDP"), stopat(0),
      b_stopat(false), applyat(0), applyat_time(0), b_applyat(false),
      lo_addr(lo_address_new(OSC_ADDR, OSC_FBPORT)), b_quit(false), f_update(1),
      t_locate(0), t_elev(0), t_apply(0), lastphi(0),
      osc_prefix(std::string("/") + name + std::string("/"))
{
  ...
  std::string s;
  // lost =
  // lo_server_thread_new_multicast(OSC_ADDR,OSC_PORT,OSC::lo_err_handler_cb);
