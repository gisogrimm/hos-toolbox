...
mixergui_t::mixergui_t() : mm(NULL), modified(true)
{
  pthread_mutex_init(&mutex, NULL);
  Glib::signal_timeout().connect(sigc::mem_fun(*this, &mixergui_t::on_timeout),
                                 125);
  ...
  //  override: signal_expose_event().connect(sigc::mem_fun(*this,
  //  &mixergui_t::on_expose_event), false);
...
void mixergui_t::hdspmm_new(MM::namematrix_t* src)
{
  pthread_mutex_lock(&mutex);
  mm = src;
  if(mm)
...
bool mixergui_t::on_draw(const Cairo::RefPtr<Cairo::Context>& cr)
{
  ...
  if(mm) {
    ...
    for(unsigned int kin = 0; kin < mm->get_num_inputs(); kin++) {
      for(unsigned int kout = 0; kout < mm->get_num_outputs(); kout++) {
        ...
        if((mm->get_n_in(kin) == 1) && (pan >= 0)) {
          ...
          // cr->move_to( xc, yc );
          cr->move_to(xc - 0.6 * r * cos(pan * 2 * M_PI),
                      yc - 0.6 * r * sin(pan * 2 * M_PI));
          cr->line_to(xc - r * cos(pan * 2 * M_PI),
                      yc - r * sin(pan * 2 * M_PI));
          cr->stroke();
          cr->restore();
        }
        ...
        for(unsigned int kc = 0; kc < mm->get_n_out(kout); kc++) {
          ...
          cr->move_to(xc - 0.9 * r * cos(pk * 2 * M_PI),
                      yc - 0.9 * r * sin(pk * 2 * M_PI));
          cr->line_to(xc - 1.1 * r * cos(pk * 2 * M_PI),
                      yc - 1.1 * r * sin(pk * 2 * M_PI));
          cr->stroke();
        }
        cr->restore();
        // end n_out
    ...
    for(unsigned int kout = 0; kout < mm->get_num_outputs(); kout++) {
      ...
      // double gain = 0.5;
      cr->save();
      cr->set_source_rgb(std::min(1.0, gain), std::min(1.0, (gain > 0) - gain),
                         0);
      cr->move_to(x1, y1);
      cr->line_to(x1, y2);
      cr->line_to(x2, y2);
