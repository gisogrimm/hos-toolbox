...
*/
  ...
  {
  public:
    mm_midicc_t(const std::string& osc_server_addr,const std::string& osc_server_port,const std::string& osc_dest_addr);
    ~mm_midicc_t();
...
mm_midicc_t::mm_midicc_t(const std::string& osc_server_addr,const std::string& osc_server_port,const std::string& osc_dest_addr)
    ...
    b_run_service(false)
{
  ...
  lo_server_thread_add_method(lost,"/hdspmm/backend_new","ii",mm_midicc_t::hdspmm_new,this);
  lo_server_thread_add_method(lost,"/hdspmm/backend_quit","",mm_midicc_t::hdspmm_destroy,this);
...
int mm_midicc_t::hdspmm_new(const char *path, const char *types, lo_arg **argv, int argc, lo_message msg, void *user_data)
{
  //DEBUG(path);
...
int mm_midicc_t::hdspmm_destroy(const char *path, const char *types, lo_arg **argv, int argc, lo_message msg, void *user_data)
{
  //DEBUG(path);
...
int main(int argc, char** argv)
{
  try{
    ...
    if( argc < 3 ){
      std::cerr << "Usage: mm_midicc <midi_dev> <midi_port> [ <osc_server_addr> <osc_server_port> <osc_dest_url> ]\n";
      return 1;
    }
