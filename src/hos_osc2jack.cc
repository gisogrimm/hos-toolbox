...
public:
  osc2jack_t(const std::string& oscad,const std::vector<std::string>& names,const std::string& multicast,const std::string& serverport,const std::string& jackname, bool use_touchosc_);
  ~osc2jack_t();
  void run();
...
int osc2jack_t::osc_upload(const char *path, const char *types, lo_arg **argv, int argc, lo_message msg, void *user_data)
{
  if( user_data )
