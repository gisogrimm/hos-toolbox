...
static double spheres_colours[6][3] = {
    {0.581818, 0.290909, 0.290909}, {0.522727, 0.581818, 0.290909},
    {0.290909, 0.581818, 0.404545}, {.290909, 0.404545, 0.581818},
    {0.522727, 0.290909, 0.581818}, {0.3, 0.3, 0.3}};
extern double brightness;
double brightness = 0.5;

void set_hoscolor(unsigned int k, const Cairo::RefPtr<Cairo::Context>& cr,
                  double alpha)
{
  k = k % 6;
  /*
...
int pos_tail_t::addpoint(const char* path, const char* types, lo_arg** argv,
...
pos_tail_t::pos_tail_t(const std::string& pAddr, lo_server_thread& lost,
                       unsigned int k)
    : rotate(0)
{
  ...
  tail.resize(100);
  tailp = tail.begin();
  lo_server_thread_add_method(lost, pAddr.c_str(), "ff", pos_tail_t::addpoint,
                              this);
  k_ = k;
}
...
visualize_t::visualize_t(const std::vector<std::string>& paddr,
                         lo_server_thread& l)
    : lost(l), names(paddr), rotate(0)
{
  ...
  Glib::signal_timeout().connect(sigc::mem_fun(*this, &visualize_t::on_timeout),
                                 100);

#ifndef GLIBMM_DEFAULT_SIGNAL_HANDLERS_ENABLED
  // Connect the signal handler if it isn't already a virtual method override:
  // signal_expose_event().connect(sigc::mem_fun(*this,
  // &visualize_t::on_expose_event), false);
